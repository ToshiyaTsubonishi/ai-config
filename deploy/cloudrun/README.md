...
このディレクトリ直下の YAML は、現行の固定 topology を記録した reference assets です。
production と別 project に staging stack を立てる場合は、固定 YAML を直接コピーするのではなく、
`staging/` 配下の renderable assets を使ってください。

- `staging/stack.example.yaml`
  `abiding-aspect-457603-m8` 向けの default values
- `staging/render_stack.py`
  project number / image / secret 名を注入して Cloud Run YAML と
  `OPENWEBUI_TOOL_SERVER_CONNECTIONS` JSON を生成する renderer
- `staging/apply_rendered_stack.sh`
  render 後の 5 service を順番に apply する helper
- `staging/README.md`
  CLI ベースの staging deploy guide
- `staging/gcp-gui-setup-guide.ja.md`
  GCP Console ベースの staging guide
...
production 環境の中で build しません。staging などで成功した repo state を
別の build-capable environment で GHCR に publish し、その pinned digest を
Cloud Run に渡します。

- `release/publish_ghcr_release.py`
  selector/provider をまとめて build し、`ghcr-release-manifest.json` を生成する
- `release/README.md`
  local publish と GitHub Actions の手順、package visibility の注意点
- `.github/workflows/publish-ghcr-release.yml`
  `ai-config-provider` checkout つきの GHCR publish workflow
...
生成された manifest の `cloudrun.images.selector` / `cloudrun.images.provider` と
`cloudrun.provenance` を、staging renderer や production 用 manifest にそのまま移します。
production が GHCR に認証できない場合は、deploy 時だけ GitHub Packages を一時的に
`public` にして pull させ、完了後に戻してください。
...
この local build は workstation や release runner 用です。constrained production の主系は
上の GHCR release path で、Cloud Run に貼る image URL は tag ではなく `@sha256:` の
digest ref を使います。
...
Open WebUI の `TOOL_SERVER_CONNECTIONS` は OpenAPI/MCPO protocol を前提にしているため、`ai-config-provider` を Open WebUI から直接つなぐのではなく、provider の `/mcp` を別の MCPO で OpenAPI 化して公開します。
...
このディレクトリには、Open WebUI から `ai-config` selector と `ai-config-provider` を MCPO 経由で使うための Cloud Run テンプレートも置いています。

GUI 中心で進める詳しい手順は `gcp-gui-setup-guide.ja.md` を参照してください。
separate-project staging は `staging/gcp-gui-setup-guide.ja.md` を参照してください。
constrained production 用の prebuilt GHCR path は `release/README.md` を参照してください。
...
- `ai-config-provider.service.yaml`
  `ai-config-provider` を `/mcp` / `/healthz` / `/readyz` つきで公開する Cloud Run Service
...
- `ai-config-provider-mcpo.service.yaml`
  `ghcr.io/open-webui/mcpo:v0.0.20` で `ai-config-provider` の `/mcp` を OpenAPI 化する Cloud Run Service
...
- `open-webui.tool-server-connections.example.json`
  `OPENWEBUI_TOOL_SERVER_CONNECTIONS` secret に入れる JSON のサンプル。selector 用 MCPO と provider 用 MCPO の 2 接続を含みます

この構成では Cloud Run の公開面は次の 4 つです。
...
- `https://ai-config-provider-mcpo-424287527578.asia-northeast1.run.app`

Open WebUI は `ai-config-mcpo` と `ai-config-provider-mcpo` の 2 つだけを OpenAPI tool server として参照します。
...
`OPENWEBUI_TOOL_SERVER_CONNECTIONS` は `open-webui.tool-server-connections.example.json` をベースに、すべての entry の `key` を実際の MCPO key に置き換えて作成します。
...
for entry in payload:
    entry["key"] = __import__("os").environ["MCPO_API_KEY"]
...
`ai-config-provider` は別 repo なので、provider image は `../ai-config-provider` 側で作ります。Cloud Run image へ actual files を入れるため、先に `provider-bundle` を materialize してから build します。
...
gcloud builds submit \
  --project "$PROJECT_ID" \
  --tag "${REGION}-docker.pkg.dev/${PROJECT_ID}/ghcr/ai-config-provider:main" \
  .
...
gcloud run services replace deploy/cloudrun/ai-config-provider.service.yaml \
  --project "$PROJECT_ID" \
  --region "$REGION"
...
gcloud run services replace deploy/cloudrun/ai-config-provider-mcpo.service.yaml \
  --project "$PROJECT_ID" \
  --region "$REGION"
...
`ai-config-provider` の health / readiness:

```bash
curl -fsS "https://ai-config-provider-424287527578.asia-northeast1.run.app/healthz"
curl -fsS "https://ai-config-provider-424287527578.asia-northeast1.run.app/readyz"
```

selector 用 MCPO の OpenAPI schema:
...
provider 用 MCPO の OpenAPI schema:

```bash
curl -fsS \
  -H "Authorization: Bearer ${MCPO_API_KEY}" \
  "https://ai-config-provider-mcpo-424287527578.asia-northeast1.run.app/openapi.json"
```
...
- Open WebUI では `ai-config (MCPO)` と `ai-config-provider (MCPO)` の両方を `＋` メニューから有効化します
