...
このガイドを最後まで終えると、Open WebUI の入力欄横にある `＋` メニューから `ai-config (MCPO)` と `ai-config-provider (MCPO)` を有効化できるようになります。

やることは大きく 6 つです。

1. `ai-config-selector-serving` のコンテナイメージを用意する
2. `ai-config-provider` のコンテナイメージを `provider-bundle` 付きで用意する
3. Cloud Run に `ai-config-selector` と `ai-config-provider` を作る
4. Cloud Run に `ai-config-mcpo` と `ai-config-provider-mcpo` を作る
5. 既存の `open-webui` サービスに接続設定を追加する
6. Open WebUI 画面から接続できることを確認する

このガイドは「できるだけ GCP コンソール GUI で進めたい」人向けに書いています。今回の会社環境を前提に、GitHub を GCP に接続しない、`gcloud` は使いません、という条件で組み直しています。

つまり、この版のガイドでは GCP の中で build はしません。`ai-config-selector-serving` と `ai-config-provider` は事前にコンテナイメージを用意して、そのイメージ URL を Cloud Run に入れる方式で進めます。

## 0. 先に全体像をつかむ

今回の接続は次の流れです。

1. Open WebUI は `ai-config-mcpo` と `ai-config-provider-mcpo` に接続する
2. `ai-config-mcpo` は `ai-config-selector` の `/mcp` に接続する
3. `ai-config-provider-mcpo` は `ai-config-provider` の `/mcp` に接続する
4. `ai-config-provider` は `ai-config-selector` の `/catalog/tool-detail` と local provider bundle を使って実体をロードする
...
- `ai-config-provider`
  `ai-config-provider` を Cloud Run で動かすサービスです。selector の結果を受けて actual loader plane を担当します
- `ai-config-mcpo`
  MCPO を Cloud Run で動かすサービスです。Open WebUI から selector 用に使います
- `ai-config-provider-mcpo`
  MCPO を Cloud Run で動かすサービスです。Open WebUI から provider 用に使います
...
| ai-config provider のサービス名 | `ai-config-provider` |
| selector 用 MCPO のサービス名 | `ai-config-mcpo` |
| provider 用 MCPO のサービス名 | `ai-config-provider-mcpo` |
...
| ai-config provider image URL | あなたが事前に用意したイメージ URL |
...
| ai-config provider URL | `https://ai-config-provider-424287527578.asia-northeast1.run.app` |
| ai-config provider MCP URL | `https://ai-config-provider-424287527578.asia-northeast1.run.app/mcp` |
| selector 用 MCPO URL | `https://ai-config-mcpo-424287527578.asia-northeast1.run.app` |
| provider 用 MCPO URL | `https://ai-config-provider-mcpo-424287527578.asia-northeast1.run.app` |
...
## 3. `ai-config-selector-serving` と `ai-config-provider` のイメージを事前に用意する

ここが今回のいちばん大事な前提です。

このガイドでは GitHub を GCP に接続しない、`gcloud` は使いません、という条件なので、GCP コンソールの中だけでこの repo の Dockerfile を build する流れは main route にしません。

その代わり、`ai-config-selector-serving` と `ai-config-provider` は事前にコンテナイメージを用意しておきます。
...
```text
ghcr.io/your-org/ai-config-provider:main
```
...
```text
docker.io/your-org/ai-config-provider:main
```
...
```text
asia-northeast1-docker.pkg.dev/sbi-art-auction/cloud-run/ai-config-provider:main
```

### 3-2. まず `ai-config-selector-serving` を build して push する
...
push が成功したら、Cloud Run に入れる selector image URL は次になります。
...
### 3-3. 次に `ai-config-provider` を build して push する

ここだけ selector と違って 1 つ追加作業があります。`ai-config-provider` image には actual files を入れた `provider-bundle` が必要です。

まず `ai-config-provider` repo で bundle を作ります。
...
この `provider-bundle/` が Cloud Run 上の `ai-config-provider` から実際に読まれる payload です。

その後で image を build します。

```bash
docker build -t ai-config-provider:local .
```

そして selector と同じ考え方で、使えるレジストリへ push します。

例:

```bash
docker tag ai-config-provider:local ghcr.io/YOUR_GITHUB_USERNAME/ai-config-provider:main
docker push ghcr.io/YOUR_GITHUB_USERNAME/ai-config-provider:main
```

または:

```bash
docker tag ai-config-provider:local asia-northeast1-docker.pkg.dev/sbi-art-auction/REPOSITORY/ai-config-provider:main
docker push asia-northeast1-docker.pkg.dev/sbi-art-auction/REPOSITORY/ai-config-provider:main
```

### 3-4. どのイメージ URL を使えばいいかわからないとき
...
この guide の残りは、selector 用 image URL と provider 用 image URL の 2 本が決まっていれば、そのまま進められます。
...
Open WebUI は、MCPO への接続設定を JSON で受け取ります。今回は selector 用 MCPO と provider 用 MCPO の 2 接続を入れます。
...
この JSON では 2 つの entry の `key` を、さっき作った `MCPO_API_KEY` の実値に置き換えてください。
...
## 6. Cloud Run に `ai-config-provider` を作る

次は provider runtime です。ここは selector の結果を受けて actual loader plane を担当します。
...
| 項目 | 値 |
|---|---|
| サービス名 | `ai-config-provider` |
| リージョン | `asia-northeast1` |
| コンテナ イメージ URL | 3 章で決めた `ai-config-provider` のイメージ URL |
...
この最小構成では provider も public にして、Open WebUI の入口認証は MCPO の API key に寄せます。
...
| コマンド | 空欄のままで OK |
| 引数 | 空欄のままで OK |
| `AI_CONFIG_PROVIDER_DIR` | `/app/provider-bundle` |
| `AI_CONFIG_SELECTOR_BASE_URL` | `https://ai-config-selector-424287527578.asia-northeast1.run.app` |
| メモリ | `1 GiB` |
| CPU | `1` |
...
| サービス アカウント | `open-webui-runner@sbi-art-auction.iam.gserviceaccount.com` |
| Liveness | HTTP GET `/healthz` on `8080` |
| Startup | HTTP GET `/readyz` on `8080` |

### 6-5. デプロイして確認する

1. `作成` もしくは `デプロイ` を押します
2. `https://ai-config-provider-424287527578.asia-northeast1.run.app/healthz` が `{"status":"ok"}` になることを確認します
3. `https://ai-config-provider-424287527578.asia-northeast1.run.app/readyz` が `200` になることを確認します

## 7. Cloud Run に `ai-config-mcpo` を作る

次は selector 用 MCPO です。Open WebUI はこのサービスを selector の入口として見に行きます。
...
## 8. Cloud Run に `ai-config-provider-mcpo` を作る

こちらは provider 用 MCPO です。Open WebUI は provider 自体ではなく、この MCPO を OpenAPI tool server として見に行きます。
...
| コマンド | `/bin/sh` |
| 引数 1 | `-c` |
| 引数 2 | `exec mcpo --host 0.0.0.0 --port "${PORT:-8080}" --api-key "$MCPO_API_KEY" --strict-auth --server-type "streamable-http" -- "$AI_CONFIG_PROVIDER_MCP_URL"` |

### 8-4. 環境変数と Secret を設定する

通常の環境変数:

| 名前 | 値 |
|---|---|
| `AI_CONFIG_PROVIDER_MCP_URL` | `https://ai-config-provider-424287527578.asia-northeast1.run.app/mcp` |
...
1. `作成` もしくは `デプロイ` を押します
2. Cloud Run のログにエラーが出ていないことを確認します
3. できればブラウザで `https://ai-config-provider-mcpo-424287527578.asia-northeast1.run.app` を開きます

これも `--strict-auth` を付けているので、そのままでは OpenAPI ドキュメントは見えないか、認証エラーになります。これは正常です。

## 9. 既存の `open-webui` サービスに接続設定を追加する
...
この 1 行に selector 用 MCPO と provider 用 MCPO の両方が入ります。
...
### 10-2. `ai-config (MCPO)` と `ai-config-provider (MCPO)` を有効化する

1. 新しいチャットを開きます
2. 入力欄の横にある `＋` を押します
3. Global Tool Servers の一覧に `ai-config (MCPO)` と `ai-config-provider (MCPO)` が出ることを確認します
4. その 2 つを ON にします
...
```text
Cloud Run デプロイに関係する ai-config のツールを探して、必要なら ai-config-provider も使って
```

うまくいけば、Open WebUI から MCPO 経由で selector と provider の両方が呼ばれます。
...
### 症状 2. `ai-config-mcpo` または `ai-config-provider-mcpo` は動くが Open WebUI から接続できない

よくある原因:

- `AI_CONFIG_SELECTOR_MCP_URL` の値が間違っている
- `AI_CONFIG_PROVIDER_MCP_URL` の値が間違っている
- `MCPO_API_KEY` が空
...
2. `Cloud Run` → `ai-config-provider-mcpo` → `変数とシークレット`
3. `Secret Manager` → `MCPO_API_KEY`
4. `Secret Manager` → `OPENWEBUI_TOOL_SERVER_CONNECTIONS`
...
### 症状 4. `ai-config (MCPO)` または `ai-config-provider (MCPO)` が UI に見えない
...
3. Open WebUI で `＋` を押して 2 つの global tool server を ON にする
...
### 症状 6. `ai-config-provider` が Ready にならない

よくある原因:

- `provider-bundle` を作らずに image を build した
- `AI_CONFIG_SELECTOR_BASE_URL` が間違っている
- provider image URL が違う

確認する場所:

1. `Cloud Run` → `ai-config-provider` → `ログ`
2. `Cloud Run` → `ai-config-provider` → `変数`
3. provider image を build した作業ログで `npm run bundle:from-ai-config -- --ai-config-dir ../ai-config --output-dir provider-bundle` を実行したか
...
- `ai-config-provider` の image URL が 1 本決まっている
- Cloud Run の `ai-config-selector` が `Ready`
- Cloud Run の `ai-config-provider` が `Ready`
- Cloud Run の `ai-config-mcpo` が `Ready`
- Cloud Run の `ai-config-provider-mcpo` が `Ready`
...
- Open WebUI 画面の `＋` から `ai-config (MCPO)` と `ai-config-provider (MCPO)` を ON にできる

## 13. この guide で参照している repo 内ファイル
