...
1. `ai-config` 側の `src/ai_config/dispatch/*` から runtime proxy module を削除し、import guard だけを残す
...
- `ai-config` の `dispatch` は案内専用の import guard だけを残す
...
|---|---|---|
| local dev | `AI_CONFIG_DISPATCH_CMD` -> sibling checkout -> installed `ai-config-dispatch` -> installed `python -m ai_config_dispatch.cli` -> fail fast | silent hard failure without diagnostics |
| GCP production | `AI_CONFIG_DISPATCH_CMD` -> installed `ai-config-dispatch` -> in-image `python -m ai_config_dispatch.cli` -> fail fast | sibling checkout, any in-repo runtime path |

### Production rules

- Cloud Run / GCP production では sibling checkout を探索しない
- in-repo compatibility runtime は存在しない前提にする
...
4. in-repo fallback は削除し、runtime 未導入時は明示エラーで fail-fast する
...
- GCP production 相当 env で sibling checkout や in-repo runtime path が選ばれない
...
PYTHONPATH=src .venv/bin/python -m pytest tests/test_plan_boundary.py tests/test_dispatch_import_guard.py -q
