...
$specs = @(
  [pscustomobject]@{
    ...
    Variables = @(
      ...
      [pscustomobject]@{ Key = "APIFY_TOKEN"; Required = $false; Secret = $true; Prompt = "Apify token" },
      [pscustomobject]@{ Key = "BOX_CLIENT_ID"; Required = $false; Secret = $true; Prompt = "Box client ID" },
      [pscustomobject]@{ Key = "BOX_CLIENT_SECRET"; Required = $false; Secret = $true; Prompt = "Box client secret" },
      [pscustomobject]@{ Key = "BOX_ENTERPRISE_ID"; Required = $false; Secret = $true; Prompt = "Box enterprise ID" },
      [pscustomobject]@{ Key = "BOX_USER_ID"; Required = $false; Secret = $true; Prompt = "Box user ID" },
      [pscustomobject]@{ Key = "BOX_PUBLIC_KEY_ID"; Required = $false; Secret = $true; Prompt = "Box public key ID" },
      [pscustomobject]@{ Key = "BOX_PRIVATE_KEY"; Required = $false; Secret = $true; Prompt = "Box private key" },
      [pscustomobject]@{ Key = "BOX_PASSPHRASE"; Required = $false; Secret = $true; Prompt = "Box private key passphrase" },
      [pscustomobject]@{ Key = "BOX_APP_SETTINGS_JSON"; Required = $false; Secret = $true; Prompt = "Box app settings JSON" },
      [pscustomobject]@{ Key = "BOX_ACCESS_TOKEN"; Required = $false; Secret = $true; Prompt = "Box access token (if using token auth)" },
      [pscustomobject]@{ Key = "FIREBASE_TOKEN"; Required = $false; Secret = $true; Prompt = "Firebase token" },
      [pscustomobject]@{ Key = "GOOGLE_MAPS_API_KEY"; Required = $false; Secret = $true; Prompt = "Google Maps API key" },
      [pscustomobject]@{ Key = "LINE_CHANNEL_ACCESS_TOKEN"; Required = $false; Secret = $true; Prompt = "LINE channel access token" },
      [pscustomobject]@{ Key = "LINE_DESTINATION_USER_ID"; Required = $false; Secret = $true; Prompt = "LINE destination user ID" },
      ...
      [pscustomobject]@{ Key = "WEBFLOW_MCP_ARGS"; Required = $false; Secret = $false; Prompt = "Webflow MCP args override as JSON/TOML array text (optional)" },
      ...
      [pscustomobject]@{ Key = "SQLITE_DB_PATH"; Required = $false; Secret = $false; Prompt = "SQLite DB path for MCP" },
      [pscustomobject]@{ Key = "MCP_LOCAL_TIMEZONE"; Required = $false; Secret = $false; Prompt = "MCP time server timezone (e.g. Asia/Tokyo)" },
