...
def test_load_external_module_emits_deprecation_warning(tmp_path: Path, monkeypatch) -> None:
    external_src = tmp_path / "ai-config-dispatch" / "src"
    package_dir = external_src / "ai_config_dispatch"
    package_dir.mkdir(parents=True)
    (package_dir / "__init__.py").write_text("", encoding="utf-8")
    (package_dir / "dummy_warning.py").write_text("VALUE = 9\n", encoding="utf-8")

    monkeypatch.setattr(_compat, "_EXTERNAL_REPO_SRC", external_src)
    _compat._WARNED_MODULES.clear()
    sys.modules.pop("ai_config_dispatch", None)
    sys.modules.pop("ai_config_dispatch.dummy_warning", None)

    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always", DeprecationWarning)
        module = _compat.load_external_module("dummy_warning")

    assert module.VALUE == 9
    assert any("deprecated compatibility shim" in str(item.message) for item in caught)
